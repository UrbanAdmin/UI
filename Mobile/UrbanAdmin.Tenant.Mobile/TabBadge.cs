...
// 013-tenant-pagos-alertas-redesign (research.md §7): .NET MAUI Shell has no built-in tab badge, so the
// count on the tenant's "Alertas" tab is drawn through the platform tab bar itself
// (Android BottomNavigationView badge, iOS/MacCatalyst UITabBarItem.BadgeValue). The count comes from
// AlertsBadgeState (Core, unit-tested); this class only paints it and never throws - a missing tab bar
...
public static class TabBadge
{
    ...
    private static void ApplyAndroid(int count)
    {
        ...
        var nav = decor is null ? null : FindBottomNavigation(decor);
        if (nav is null)
        {
            return;
        }
    ...
    {
        var tabBar = Platform.GetCurrentUIViewController()?.TabBarController?.TabBar;
        var item = tabBar?.Items?.FirstOrDefault(i => i.Title == AlertasTitle);
        if (item is not null)
        {
